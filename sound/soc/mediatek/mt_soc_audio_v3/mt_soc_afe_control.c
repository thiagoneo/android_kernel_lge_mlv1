...
bool InitAfeControl(void)
{
	...
	if (mAudioInit == false) {
		...
		for (i = 0; i <= Soc_Aud_Digital_Block_MEM_HDMI; i++) {
			...
			AFE_Mem_Control_context[i]->mWaitForIRQ= false; //fmx_added for Monkey System blacklist AAAA Add this line 10/24
		}
...
bool RemoveMemifSubStream(Soc_Aud_Digital_Block MemBlock, struct snd_pcm_substream *substream)
{
	...
	substreamList *temp = NULL;
	unsigned long flags;
    uint8 time_out_cnt=2; //fmx_added for Monkey System blacklist AAAA Add this line 10/24
    uint8 i =0;
	spin_lock_irqsave(&AFE_Mem_Control_context[MemBlock]->substream_lock, flags);
//fmx_added for Monkey System blacklist AAAA Add this line 10/24 begin
      for (i = 0; i < time_out_cnt; i++) {
         if (AFE_Mem_Control_context[MemBlock]-> mWaitForIRQ == 0) {
            break;
             }else {
            mdelay(5);
           }
            }
//fmx_added for Monkey System blacklist AAAA Add this line 10/24 end
...
void Auddrv_DAI_Interrupt_Handler(void)
{
	...
	if (Mem_Block->substreamL != NULL) {
		if (Mem_Block->substreamL->substream != NULL) {
			Mem_Block->mWaitForIRQ = true; //fmx_added for Monkey System blacklist AAAA Add this line 10/24 Add this line
			spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
			snd_pcm_period_elapsed(Mem_Block->substreamL->substream);
			Mem_Block->mWaitForIRQ = false;//fmx_added for Monkey System blacklist AAAA Add this line 10/24 Add this line
			spin_lock_irqsave(&Mem_Block->substream_lock, flags);
		}
	}
