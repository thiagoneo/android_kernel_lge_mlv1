 ...
 */
int g_temp_win = 0;
int g_battery_temp = 0;
int g_battery_high = 0;
extern signed int gFG_HW_voltage;
extern signed int gFG_cust_10_volt;
extern signed int gFG_cust_20_volt;
extern signed int gFG_cust_40_volt;
extern int hq_boot_val;
extern char *battery_id;
...
#define FEATURE_HQ_CURRENT_AUTO_TEST
#if defined(FEATURE_HQ_CURRENT_AUTO_TEST)
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		#if defined(CONFIG_HQ_PROJECT_ZAL1506)
		val->intval = data->BAT_BatteryAverageCurrent;
		#else
		  val->intval = data->BAT_BatteryAverageCurrent;//zhou gongmo show instant current
		#endif
		break;
...
static ssize_t show_ntc_stop(struct device *dev, struct device_attribute *attr, char *buf)
{
	
	unsigned int ret_value = 1;
	ret_value = sprintf(buf, "%s %s", "NTC_stop status:",(HQ_Close_Ntc)?"open ":"close ");	 
	return ret_value;
}

static ssize_t store_ntc_stop(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{	
	sscanf(buf , "%d" , &HQ_Close_Ntc);
	printk("zhou HQ_Close_Ntc %d\n", HQ_Close_Ntc);
	return size;
}

static DEVICE_ATTR(ntc_stop, 0664, show_ntc_stop, store_ntc_stop);
//for close ntc and set cv
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (BMT_status.bat_full == KAL_TRUE) {	/* charging full first, UI tracking to 100% */
		...
		if ((BMT_status.UI_SOC - BMT_status.SOC) > 20 && is_uisoc_ever_100 == KAL_TRUE) {
...
static kal_bool mt_battery_nPercent_tracking_check(void)
{
	...
	if ((BMT_status.ZCV <= gFG_cust_40_volt + 20 && BMT_status.UI_SOC > 40)||
	    (BMT_status.ZCV <= gFG_cust_20_volt + 10 && BMT_status.UI_SOC > 20)||
		(BMT_status.ZCV <= gFG_cust_10_volt + 30 && BMT_status.UI_SOC > 10)) {
	...
	battery_log(BAT_LOG_CRTI,"[nPercent_track] ZCV=%d UI_SOC=%d, 40_volt=%d, 20_volt=%d, 10_volt=%d,resetBatteryMeter=%d,timer_counter=%d\n",
		BMT_status.ZCV, BMT_status.UI_SOC,gFG_cust_40_volt,gFG_cust_20_volt,gFG_cust_10_volt,resetBatteryMeter,timer_counter);
#endif
	return resetBatteryMeter;
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
	...
	static int gFG_count = 0;
	if (BMT_status.UI_SOC <= 0) {
		BMT_status.UI_SOC = 0;
	} else {
		if ((BMT_status.bat_vol > 3150 || gFG_HW_voltage > SYSTEM_OFF_VOLTAGE) && BMT_status.UI_SOC > 1 ) {
			gFG_count++;
			if(gFG_count >= 6) {
				BMT_status.UI_SOC--;
				gFG_count = 0;
		...
		else if (BMT_status.bat_vol <= 3150 || gFG_HW_voltage <= SYSTEM_OFF_VOLTAGE)
			BMT_status.UI_SOC--;
...
static void battery_update(struct battery_data *bat_data)
{
...
#ifdef FEATURE_HQ_CURRENT_AUTO_TEST
	...
	if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) {
		...
		#if 0
			if (BMT_status.bat_vol <= 3300  || gFG_HW_voltage <= batt_cust_data.v_0percent_tracking)
...
#endif
        battery_log(BAT_LOG_CRTI, "[BATTERY] set_rtc_spare_fg_value and Set Battery UISOC:%d!!\n\r",BMT_status.SOC);  
	...
	// Vinton added for charger led
	{
		...
		if(BMT_status.UI_SOC >= 90){
			if(led_flag == 3)
			{
				ret = gpio_direction_output(HQ_CHARGER_LED_GPIO, 1);
				__gpio_set_value(HQ_CHARGER_LED_GPIO, 1);
				printk("[@@Vinton] led_gpio ret = %d\n", ret);
			}
			if(led_flag <= 5)
				++led_flag;
		}
	...
	bat_data->BAT_BatteryAverageCurrent = BMT_status.ICharging;
	if(BMT_status.charger_type==4 && bat_data->BAT_BatteryAverageCurrent ==0 && bat_data->BAT_STATUS != POWER_SUPPLY_STATUS_FULL)
	{
			ichg = battery_meter_get_charging_current();
			bat_data->BAT_BatteryAverageCurrent =(AC_CHARGER_CURRENT / 100*29+ichg)/30;
	}
		if(BMT_status.charger_type==1 && bat_data->BAT_BatteryAverageCurrent ==0 && bat_data->BAT_STATUS != POWER_SUPPLY_STATUS_FULL)
	{
			ichg = battery_meter_get_charging_current();
			bat_data->BAT_BatteryAverageCurrent =(USB_CHARGER_CURRENT / 100*29+ichg)/30;
	}
	#endif
...
#ifdef CONFIG_HQ_PROJECT_ZAL1066_LG
	if (BMT_status.temperature > batt_cust_data.max_charge_temperature || (BMT_status.temperature > 45 && BMT_status.bat_vol > 4000)) {
#else
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI,
	"AvgVbat=%d,bat_vol=%d,AvgI=%d,ICharging=%d,ICharging_fg=%d,VChr=%d,AvgT=%d,temperature=%d,SOC=%d,BMT_status.SOC=%d,UI_SOC=%d,ZCV=%d bcct:%d:%d I:%d hq_boot_val:%d\n",
		BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,BMT_status.ICharging_fg,
		BMT_status.charger_vol, BMT_status.temperature, temperature,
		previous_SOC, BMT_status.SOC, BMT_status.UI_SOC, BMT_status.ZCV,
		g_bcct_flag, get_usb_current_unlimited(), get_bat_charging_current_level(),hq_boot_val);
...
static void mt_battery_notify_VBatTemp_check(void)
{
	...
	#ifdef CONFIG_HQ_PROJECT_ZAL1066_LG
		if ((BMT_status.temperature >= 58 && BMT_status.temperature <= 60)) {
	#else
		if (BMT_status.temperature >= batt_cust_data.max_charge_temperature) {
	#endif
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of range(too high)\n",
			    BMT_status.temperature);
	} else if(g_temp_win == 1){
		if (BMT_status.temperature < 58){
			g_BatteryNotifyCode |= 0x0080;
			battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) is in normal range\n",
			    	BMT_status.temperature);
			g_temp_win = 0;	
			}
		}
...
#ifdef CONFIG_HQ_PROJECT_ZAL1066_LG
	if(BMT_status.bat_vol <= 4000 &&  BMT_status.temperature >= 56 && BMT_status.charger_exist == KAL_TRUE){
		g_BatteryNotifyCode |= 0x0040;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) is over 56, bat_vol <= %d\n",
				BMT_status.temperature,BMT_status.bat_vol);	
		g_battery_temp = 1;
		}else if(g_battery_temp == 1){
			if(BMT_status.temperature < 56){
				g_BatteryNotifyCode |= 0x0080;
				battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d)  bat_vol  %d is in normal range\n",
					BMT_status.temperature,BMT_status.bat_vol);
				g_battery_temp = 0;
				}
		}
	if(BMT_status.bat_vol > 4000 && BMT_status.temperature > 45 && BMT_status.charger_exist == KAL_TRUE) {
		g_BatteryNotifyCode |= 0x0040;
		battery_log(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) is over 45 BMT_status.bat_vol > %d\n",
				BMT_status.temperature,BMT_status.bat_vol);	
		g_battery_high = 1;
		}else if(g_battery_high == 1){
...
CHARGER_TYPE mt_charger_type_detection(void)
{
	...
	BMT_status.charger_type = CHR_Type_num;
		...
		if (BMT_status.charger_type != STANDARD_HOST && BMT_status.charger_type != STANDARD_CHARGER){
			//zhoucong 2016 12 06 STANDARD_CHARGER recognize NONSTANDARD_CHARGER
#else
...
static void hq_get_battery_id_name(void) {

	switch(hq_boot_val){
		case 187:
			battery_id = "SW3800_VC0";
			break;
		case 204:
			battery_id = "SW3800_VC1";
			break;
		case 120:
			battery_id = "ISL6296A_C";
			break;
		default :
			battery_id = "NULL";
			break;
...
unsigned int charging_enable = KAL_FALSE;
...
#ifdef CONFIG_HQ_RUNIN_SUPPORT
	{
		...
		//3->On  ;  2->Off
        battery_log(BAT_LOG_CRTI, "[Runin][BATTERY] BAT_STATUS_READ adc_cali_ioctl battery_in_data[0]:%d!!\n\r", battery_in_data[0]); 
		if(battery_in_data[0]==3)
		{
            battery_log(BAT_LOG_CRTI, "[Runin][BATTERY] adc_cali_ioctl cmd is 3 and turn on chg!!\n\r");  
		...
		{
            battery_log(BAT_LOG_CRTI, "[Runin][BATTERY] adc_cali_ioctl cmd is 2 and turn off chg!!\n\r");  
		...
		{
            battery_log(BAT_LOG_CRTI, "[Runin][BATTERY] adc_cali_ioctl cmd is 1 and exit runin battery test!!\n\r");  
			g_runin_test_enter = 0;
            g_runin_battery_test_flag =1;
			pchr_turn_on_charging();
		}
		...
		{
            battery_log(BAT_LOG_CRTI, "[Runin][BATTERY] Not in runin battery test mode!!\n\r");  
			g_runin_test_enter = 0;
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_ntc_stop);
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT) || defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
...
static int battery_pm_suspend(struct device *device)
{
	...
	battery_log(BAT_LOG_CRTI, "battery_pm_suspend\n");//zhoucong 2016-12-21 add for third app influnce battery meter ALPS03078906
	battery_timer_pause();
...
static int battery_pm_resume(struct device *device)
{
	...
	battery_log(BAT_LOG_CRTI, "battery_pm_resume\n");//zhoucong 2016-12-21 add for third app influnce battery meter ALPS03078906
...
static int battery_pm_event(struct notifier_block *notifier, unsigned long pm_event, void *unused)
{
	...
	case PM_SUSPEND_PREPARE:	/* Going to suspend the system */
		...
		//battery_timer_pause();//zhoucong 2016-12-21 add for third app influnce battery meter ALPS03078906
		return NOTIFY_DONE;
	case PM_POST_HIBERNATION:	/* Hibernation finished */
	case PM_POST_SUSPEND:	/* Suspend finished */
	case PM_POST_RESTORE:	/* Restore failed */
		pr_debug("[%s] pm_event %lu\n", __func__, pm_event);
		//battery_timer_resume();//zhoucong 2016-12-21 add for third app influnce battery meter ALPS03078906
		return NOTIFY_DONE;
	}
	return NOTIFY_OK;
}
