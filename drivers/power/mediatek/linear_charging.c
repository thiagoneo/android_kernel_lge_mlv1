...
{
	int cup_temp =mtkts_bts_get_hw_temp()/1000;
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			else if(ZAL1066_HQ_DEC_CUR == 1) {
					if(cup_temp > 35 && cup_temp < 43) {
						g_temp_CC_value = CHARGE_CURRENT_700_00_MA;
					}
					else if(cup_temp >= 43 ) {
...
static unsigned int charging_full_check(void)
{
	...
	if (BMT_status.ICharging_fg <= charging_full_current && BMT_status.ICharging <= charging_full_current + 300) {
		full_check_count++;
		if (6 == full_check_count) {
			status = KAL_TRUE;
			full_check_count = 0;
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery full and disable charging, ICharging = %d mA, ICharging_fg = %d mA\n",
				    BMT_status.ICharging,BMT_status.ICharging_fg);
		}
	} else {
		full_check_count = 0;
	}
...
#if 1//defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT)	/* defined(MTK_LINEAR_CHARGER_NO_DISCHARGE) */
	/* no disable charging */
#else
	{
...
if(g_runin_battery_test_flag ==1)
   BMT_status.bat_charging_state = CHR_CC; //runin
#endif
