...
signed int gFG_cust_10_volt = 3667;
signed int gFG_cust_20_volt = 3727;
signed int gFG_cust_40_volt = 3788;
signed int gFG_BATT_CAPACITY_init_high_current = 1200;
signed int gFG_BATT_CAPACITY_aging = 1200;
static int batt_zcv = 0;
...
#if (BAT_NTC_68 == 1)
	batt_meter_cust_data.bat_ntc = 68;
#endif
...
void dod_init(void)
{
	...
	//zhoucong add for zal1066 restart and poweroff test
	gFG_capacity_by_v = gFG_capacity_by_v_init;
	//zhoucong add for zal1066 restart and poweroff test
...
if (((g_rtc_fg_soc != 0)
	...
	{
	//zhoucong add for zal1066 restart and poweroff test
	printk("zhoucong0:dod_init:gFG_capacity_by_v=%d,g_rtc_fg_soc=%d,gFG_voltage_init=%d\n",gFG_capacity_by_v,g_rtc_fg_soc,gFG_voltage_init);
		if(gFG_capacity_by_v > g_rtc_fg_soc && g_rtc_fg_soc < 100){
			if(gFG_capacity_by_v - g_rtc_fg_soc > 15){
				gFG_capacity_by_v = g_rtc_fg_soc + 3;
			}else if(gFG_capacity_by_v - g_rtc_fg_soc > 8){
				gFG_capacity_by_v = g_rtc_fg_soc + 2;
			}else
				gFG_capacity_by_v = g_rtc_fg_soc + 1;
		}
		else if(gFG_capacity_by_v < g_rtc_fg_soc && g_rtc_fg_soc > 1){
			if(g_rtc_fg_soc - gFG_capacity_by_v >= 10){
				gFG_capacity_by_v = g_rtc_fg_soc - 4;
			}else if(g_rtc_fg_soc - gFG_capacity_by_v >= 7){
				gFG_capacity_by_v = g_rtc_fg_soc - 3;
			}else if(g_rtc_fg_soc - gFG_capacity_by_v >= 4){
				gFG_capacity_by_v = g_rtc_fg_soc - 2;
			}else
				gFG_capacity_by_v = g_rtc_fg_soc - 1;
		}
			...
			gFG_capacity_by_v = g_rtc_fg_soc;
	printk("zhoucong1:dod_init:gFG_capacity_by_v=%d,g_rtc_fg_soc=%d, =%d\n",gFG_capacity_by_v,g_rtc_fg_soc,gFG_voltage_init);
	//zhoucong add for zal1066 restart and poweroff test
	}
	...
	if (batt_meter_cust_data.change_tracking_point) {
		gFG_cust_10_volt = fgauge_read_v_by_capacity(90);
		gFG_cust_20_volt = fgauge_read_v_by_capacity(80);
		gFG_cust_40_volt = fgauge_read_v_by_capacity(60);
						      ...
						      signed int recursion_time)
{
	...
	if (gFG_Is_Charging == KAL_TRUE)
		batt_zcv = ori_voltage - gFG_resistance_bat*gFG_current/1000/10;
	else
		batt_zcv = ori_voltage + gFG_resistance_bat*gFG_current/1000/10;
...
/* 1. Get Raw Data */
	...
	BMT_status.ICharging_fg = gFG_current/10;
...
/* 5. Logging */
	printk("\n[FGADC] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
		 gFG_Is_Charging, gFG_current, gFG_columb, gFG_voltage, batt_zcv, gFG_capacity_by_v,
...
#endif
/* 5. Logging */
	printk("[FGADC]init: %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
...
signed int battery_meter_get_battery_nPercent_zcv(void)
{
	...
	return 3700;
#else
	return 3700;	/* 15% zcv,  15% can be customized by 100-g_tracking_point */
#endif
}
