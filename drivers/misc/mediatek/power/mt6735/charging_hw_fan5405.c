...
static u32 charging_hw_init(void *data)
{
	...
	gpio_request(GPIO106,"Switch Charger");
	gpio_direction_output(GPIO106, 1);

	if (batt_cust_data.high_battery_voltage_support){
		fan5405_reg_config_interface(0x06,0x79); // set Vsafe 4.38
		fan5405_reg_config_interface(0x02,0xb2); // 4.38
	}
	else{
		fan5405_reg_config_interface(0x06,0x70); // set Vsafe 4.20
		fan5405_reg_config_interface(0x02,0x8e); // 4.20
	}
