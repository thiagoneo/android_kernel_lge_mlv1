...
******************************************************************************/
int HQ_Close_Vib_LG = 0;//zhou add for LG test close time limitation
/* Debug message event */
...
static void vibrator_enable(struct timed_output_dev *dev, int value)
{
	...
	if(value !=0)
	{
	  vib_vol_mode = value&0xF;
	  if((vib_vol_mode >= 0x05)&&(vib_vol_mode <= 0x7))//0x04=2.0v 0x05=2.8V 0x06=3.0v 0x07=3.3v
	  {
		hq_vibr_power_set(vib_vol_mode);
	  } 
	  else //set default   vib_vol_mode=3.3v
	  {
	...
	value = value>>4;
	printk("[vibrator]vibrator_enable: vibrator after in time = %d,vib_vol_mode=%d\n", value,vib_vol_mode);
#endif
...
#if defined(CONFIG_HQ_VIBRATOR_MODE)
	printk("[vibrator]HQ_Close_Vib_LG %d\n",HQ_Close_Vib_LG);
	if(HQ_Close_Vib_LG)
	{
	printk("zhou close vib\n");//zhou add for LG test close time limitation
	}else{
		if(value < 5 || value > 6000)	
			value = 100;//ms
		}
#else
...
//zhou add for LG test close time limitation
static ssize_t show_vibr_close(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", HQ_Close_Vib_LG);
}


static ssize_t store_vibr_close(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t size)
{
	sscanf(buf , "%d", &HQ_Close_Vib_LG);
	
	return size;
}

static DEVICE_ATTR(vibr_close, 0664, show_vibr_close, store_vibr_close);
//zhou add for LG test close time limitation
...
static int vib_mod_init(void)
{
	...
	ret = device_create_file(mtk_vibrator.dev, &dev_attr_vibr_close);//zhou add for LG test close time limitation
	if (ret)
		VIB_DEBUG("device_create_file vibr_close fail!\n");
