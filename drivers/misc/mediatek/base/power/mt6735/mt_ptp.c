...
#include "mt-plat/upmu_common.h"//[PATCH] [ALPS02977636] PTP: PWM/Low Temp solution end hanwenchao
...
static void ptp_set_ptp_volt(struct ptp_det *det)
{
	...
	struct ptp_ctrl *ctrl = id_to_ptp_ctrl(det->ctrl_id);
        int tscpu_bank0_temp_is_valid = tscpu_is_temp_valid_bank0(); //[PATCH] [ALPS02977636] PTP: PWM/Low Temp solution
	...
	//[PATCH] [ALPS02977636] PTP: PWM/Low Temp solution
...
//[PATCH] [ALPS02977636] PTP: PWM/Low Temp solution
int __attribute__((weak)) tscpu_is_temp_valid_bank0(void)
{
	return 1;
}
...
static int ptp_probe(struct platform_device *pdev)
{
	...
	struct ptp_det *det;
	struct ptp_ctrl *ctrl;
	enum mt_cpu_dvfs_id cpu; // [PATCH] [ALPS02977636] PTP: PWM/Low Temp solution


	FUNC_ENTER(FUNC_LV_MODULE);

        cpu = MT_CPU_DVFS_LITTLE; //[PATCH] [ALPS02977636] PTP: PWM/Low Temp solution
...
// [PATCH] [ALPS02977636] PTP: PWM/Low Temp solution	
	/* Disable PWM mode here */    //hanwenchao
	if (mt_cpufreq_get_freq_by_idx(cpu, 0) <= 1300000) 
		pmic_force_vproc_pwm(0);   //hanwenchao
// [PATCH] [ALPS02977636] PTP: PWM/Low Temp solution
