...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "IMX2191OTP"
...
static u8 awb_data[13] = {0};
static int imx219_test_read_mode(void)
{
	u8 reVal = 0;
	int retry = 3;
	while(retry>0)
...
static void read_awb_data(void)
{
	u8  reVal;
	int addr=0;
	int retry = 3;
...
int IMX2191_UpdateLSC(void)
{
	...
	if(Checksum_Value == -1)
	{
		...
		if(Checksum_Value==1)
		{
			...
			CAM_CALDB("lili>>>imx2191  No Vaild LSC OTP Data\n");
			CAM_CALDB("lili>>>IMX2191_UpdateLSC Fail !!!!\n");                                 
		}
	}
	read_awb_data();
   return 0;
...
#endif
{
    ...
    {  
		...
		CAM_CALDB("[CAM_CAL] pWorkingBuff[0] = %d \n", pWorkingBuff[0]);
	   // i4RetValue = 222;//
...
static int CAM_CAL_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    if(g_u4Opened)
    {
		...
		printk("[S24CAM_CAL] Opened, return -EBUSY\n");
        return -EBUSY;
    }
    else
    {
...
inline static int RegisterCAM_CALCharDrv(void)
{
   ...
   CAM_CALDB("RegisterCAM_CALCharDrv\n");
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "CAM_CALdrv_IMX2191OTP");
...
static int __init OF_CAM_CAL_i2C_init(void)
{
...
static void __exit OF_CAM_CAL_i2C_exit(void)
{	
	platform_driver_unregister(&g_stCAM_CAL_Driver);
}

module_init(OF_CAM_CAL_i2C_init);
module_exit(OF_CAM_CAL_i2C_exit);
