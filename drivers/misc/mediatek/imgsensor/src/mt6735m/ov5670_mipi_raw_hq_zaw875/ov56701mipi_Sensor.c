...
#include "ov56701mipi_Sensor.h"

#ifdef CONFIG_COMPAT
#include <linux/compat.h>
#endif
#define PFX "ov56701_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV56701MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{
	LOG_INF("OV56701_Sensor_Init_2lane E\n");
...
{
	LOG_INF(" OV56701PreviewSetting_2lane enter\n");
...
{
	LOG_INF("OV56701CaptureSetting_2lane enter! currefps:%d\n",currefps);
...
static unsigned sub_camera_gpio_id_pin = (42);
...
#if 1        //Vinton add
    ret = gpio_request(sub_camera_gpio_id_pin,"front_camera_id_pin");

    if (ret < 0) {
        printk("Unable to request gpio front_camera_id_pin\n");
        gpio_free(sub_camera_gpio_id_pin);
    }
    else{
        ret = gpio_direction_input(sub_camera_gpio_id_pin);
        printk("[@@Vinton]-%s: ret = %d\n", __func__, ret);
    }    
#endif


	if(1 != gpio_get_value(sub_camera_gpio_id_pin)){
		printk("[@@Vinton] huaquan sub_camera_gpio_id_pin = %d\n", gpio_get_value(sub_camera_gpio_id_pin));
		goto SENSOR_ID_FAIL;
	}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
            ...
            printk("[@@Vinton] huaquan gpio_get_value = %d sensor_id = 0x%x.\n",
				gpio_get_value(sub_camera_gpio_id_pin), *sensor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
            printk("[@@Vinton] huaquan gpio_get_value = %d.\n",gpio_get_value(sub_camera_gpio_id_pin));
			 if(sensor_id ==0x5670)
             {
                sensor_id=0x5669;
             }
...
UINT32 OV56701_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
