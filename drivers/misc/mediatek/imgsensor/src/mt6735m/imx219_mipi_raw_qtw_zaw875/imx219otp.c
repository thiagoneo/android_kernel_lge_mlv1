...
int lsc_checksum_group11 = 0;
int lsc_checksum_group22 = 0;
...
static int Check_LSC_SUM(int Group)
{
	...
	if (Group==0) //Group 1: Page2~Page3  0x3204~0x3243  , page 4:0x3204~0x3232
	{
		...
		sum_value %= 255;
		sum_value += 1;
		printk("lili>>>>>sum_value=0x%x  >>>>lsc_checksum_group11=0x%x\n",sum_value,lsc_checksum_group11);
		if(sum_value==lsc_checksum_group11)
			return 1;
	}
	else if (Group==2) //Group 2: page 7:0x3222~0x3243 , page 8/page 9 :0x3204~0x3243 ,page 10:0x3204~0x3210)
	{
		...
		sum_value %= 255;
		sum_value += 1;
		if(sum_value==lsc_checksum_group22)
			return 1;
	}
	else
	{
...
int Find_LSC_Group0(void)
{
	u8  reVal=1;
	//int retry=3;		
	IMX219_readpage(0);
	reVal=read_otp_reg(0x3243);//read 0x3243 for lsc  group1 ? or  Group 2?
	lsc_checksum_group11 = read_otp_reg(0x3241);
	lsc_checksum_group22 = read_otp_reg(0x3242);
	printk("lili>>>0x%x \n",reVal);	
	if((((reVal)&0xF0)==0x40))
	{
		return 0;
