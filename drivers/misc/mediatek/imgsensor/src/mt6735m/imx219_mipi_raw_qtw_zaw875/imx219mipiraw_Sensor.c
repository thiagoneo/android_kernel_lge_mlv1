...
extern int IMX219_UpdateLSC(void);
extern int read_otp_module_id(void);
...
static imgsensor_struct imgsensor = {
	.mirror = IMAGE_NORMAL,//IMAGE_V_MIRROR,//IMAGE_HV_MIRROR,//IMAGE_NORMAL,				//mirrorflip information
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    printk("[IMX219MIPI]enter IMX219MIPIGain2Reg function\n");
	...
	printk("exit IMX219MIPIGain2Reg function\n");
		...
		do {
			*sensor_id = return_sensor_id();
			...
			//if (*sensor_id == imgsensor_info.sensor_id) {		
				printk("imx219_mipi_raw_qtw_zaw875:[imx219]i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			printk("imx219_mipi_raw_qtw_zaw875:[imx219]Read sensor id fail, i2c write id: 0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		module_id = read_otp_module_id();
		//("imx219_mipi_raw_qtw_zaw875:lili read otp_module_id %d\n",module_id_open);		
		do {
	...
	if (imgsensor_info.sensor_id != sensor_id ||(module_id != 0x06 ))
		return ERROR_SENSOR_CONNECT_FAIL;
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			printk("Warning: current_fps,fps is not support, so use cap1's setting=%d fps!\n",imgsensor_info.cap1.max_framerate/10);
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_CROP_INFO:
           printk("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%lld\n", *feature_data);
