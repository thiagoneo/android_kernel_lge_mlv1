...
static imgsensor_info_struct imgsensor_info = { 
	...
	.i2c_addr_table = {0x42,0xff},//0x42 0x6c record sensor support all write id addr, only supprt 4must end with 0xff
};
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =	 
{
 ...
 { 3296, 2480,    0,	12, 3296, 2456, 1648, 1228,   2,	2, 1632, 1224,	 0, 0, 1632, 1224}, // video
...
#define OV8856R1AOTP
 ...
 #ifdef OV8856R1AOTP
static int ov8856qt_otp_read_flag = 0;
 /*****************************OV8856QT OTP start***********************************/
...
static int read_otp(struct otp_struct *otp_ptr)
{
   ...
   // OTP base information and WB calibration data
   otp_flag = read_cmos_sensor(0x7010);
   LOG_INF("[OTP_Module_Info]  otp_flag: 0x%x\n", otp_flag); 
   addr = 0;
   ...
   hw_info_main_otp.vendor_id = (*otp_ptr).module_integrator_id;
   hw_info_main_otp.year = (*otp_ptr).production_year;
   hw_info_main_otp.month = (*otp_ptr).production_month;
   hw_info_main_otp.day = (*otp_ptr).production_day;
   hw_info_main_otp.lens_id = (*otp_ptr).lens_id;
   //hw_info_main_otp.vcm_id = (*otp_ptr).vcm_id;
   ...
   LOG_INF("[OTP_Module_Info] rg_ratio = 0x%x\n", (*otp_ptr).rg_ratio);
   LOG_INF("[OTP_Module_Info] bg_ratio = 0x%x\n", (*otp_ptr).bg_ratio);
   ...
   LOG_INF("[OTP_Module_Info]  otp_flag_2: 0x%x\n", otp_flag); 
   addr = 0;
   if((otp_flag & 0xc0) == 0x40) 
   {
   ...
   if(addr != 0) 
   {
     ...
     (* otp_ptr).VCM_end = (read_cmos_sensor(addr)<<2) | ((temp>>6) & 0x03);
     (* otp_ptr).VCM_start = (read_cmos_sensor(addr + 1) << 2) | ((temp>>4) & 0x03);
   ...
   else 
   {
	...
	sensor_otp_data[19] = ((* otp_ptr).VCM_start & 0xff);
	sensor_otp_data[20] = ((* otp_ptr).VCM_start & 0xff00) > 8;
	sensor_otp_data[17] = ((* otp_ptr).VCM_end & 0xff);
	sensor_otp_data[18] = ((* otp_ptr).VCM_end & 0xff00) > 8;

	
    LOG_INF("[OTP_Module_Info]  (* otp_ptr).VCM_start: 0x%x,(* otp_ptr).VCM_end: 0x%x\n", (* otp_ptr).VCM_start,(* otp_ptr).VCM_end); 
 // OTP Lenc Calibration
   otp_flag = read_cmos_sensor(0x702c);
	LOG_INF("[OTP_Module_Info]	otp_flag_3: 0x%x\n", otp_flag); 
   addr = 0;
   ...
   if(addr != 0) 
   {
     ...
     LOG_INF("[OTP_Module_Info]  checksum2: 0x%x\n",checksum2 );
	 LOG_INF("[OTP_Module_Info]  (* otp_ptr).checksum : 0x%x\n",(* otp_ptr).checksum );
...
static void sensor_init(void)
{
    ...
    */
	...
	write_cmos_sensor(0x370b, 0x80); //20160422 according to ov Fae from 0x80to 0x7e for BLC performance
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			//*sensor_id = (tmp_sensor_id = read_cmos_sensor(0x300B)) ? (tmp_sensor_id << 8 | read_cmos_sensor(0x300C)) : 0;
			...
			*sensor_id = *sensor_id + 1;
			...
			LOG_INF("Read sensor id fail, i2c_id: 0x%x, sensor_id:0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	if(ov8856qt_otp_read_flag == 0)
	{
		read_otp(&otp_ptr);
		ov8856qt_otp_read_flag = 1;
	}
