...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		.pclk = 171200000,	/* record different mode's pclk */
	...
	.hs_video = {
		     .pclk = 171200000,	/* record different mode's pclk */
		     .linelength = 2816,	/* record different mode's linelength */
		     .framelength = 1041,	/* record different mode's framelength */
		     .startx = 0,	/* record different mode's startx of grabwindow */
		     .starty = 0,	/* record different mode's starty of grabwindow */
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0, 0, 2592, 1944, 0, 0, 2592, 1944},	/* capture */
	{2592, 1944, 0, 0, 2592, 1944, 640, 480, 0, 0, 640, 480, 0, 0, 640, 480},	/* Preview */
	{2592, 1944, 16, 252, 2560, 1440, 1280, 720, 0, 0, 1280, 720, 0, 0, 1280, 720},	/* Preview */
};
...
////////////////////////////////////////////////
	...
	write_cmos_sensor(0x0b16, 0x6b0b); 
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor(0x001c, 0x1111);                               
	write_cmos_sensor(0x001e, 0x1111);                               
	write_cmos_sensor(0x0006, 0x0411); //frame length lines          
	...
	write_cmos_sensor(0x003e, 0x0000);                               
	write_cmos_sensor(0x0004, 0x040b); //coarse int. time            
	write_cmos_sensor(0x0a04, 0x016a); //isp_en                      
	write_cmos_sensor(0x0a1A, 0x0800);                               
...
static int hi553_otp_read(void)
{
	...
	checksum = hi553_otp.module_integrator_id + hi553_otp.AF_Flag + hi553_otp.prodyction_year;
	checksum = checksum + hi553_otp.production_month + hi553_otp.production_day;
	checksum = checksum + hi553_otp.sensor_id + hi553_otp.lens_id;
	...
	LOG_INF("[OTP_Module_Info] WB_FLAG = 0x%x \n", hi553_otp.WB_FLAG);
	...
	if (wb_start_addr != 0) {
		...
		for (i = 0; i < 30; i++) {
			...
			checksum += hi553_otp.wb_data[i];
			LOG_INF("[OTP_Module_Info] wb_data[%d] = 0x%x\n", i, hi553_otp.wb_data[i]);
		}

		checksum = (checksum -hi553_otp.wb_data[29]) % 0xFF +1;
		
		if (checksum == hi553_otp.wb_data[29])
		{
			LOG_INF("[OTP_Module_Info]: WB checksum PASS\n ");
		}
		else
		{
			LOG_INF("[OTP_Module_Info]: WB checksum Fail\n ");
		}
...
static int hi553_otp_apply(struct hi553_otp_struct *otp_ptr)
{
	...
	LOG_INF("[OTP_Module_Info] hi553_otp_apply \n");
	...
	//R_gain = 0x11111111;
	LOG_INF("[OTP_Module_Info] Before apply otp G_gain = 0x%x\n", (read_cmos_sensor_byte(0x0126) << 8) | (read_cmos_sensor_byte(0x0127) & 0xFFFF));
	LOG_INF("[OTP_Module_Info] Before apply otp R_gain = 0x%x\n", (read_cmos_sensor_byte(0x012a) << 8) | (read_cmos_sensor_byte(0x012b) & 0xFFFF));
	LOG_INF("[OTP_Module_Info] Before apply otp B_gain = 0x%x\n", (read_cmos_sensor_byte(0x012c) << 8) | (read_cmos_sensor_byte(0x012d) & 0xFFFF));
	...
	LOG_INF("[OTP_Module_Info] after apply otp G_gain = 0x%x\n", (read_cmos_sensor_byte(0x0126) << 8) | (read_cmos_sensor_byte(0x0127) & 0xFFFF));
	LOG_INF("[OTP_Module_Info] after apply otp R_gain = 0x%x\n", (read_cmos_sensor_byte(0x012a) << 8) | (read_cmos_sensor_byte(0x012b) & 0xFFFF));
	LOG_INF("[OTP_Module_Info] after apply otp B_gain = 0x%x\n", (read_cmos_sensor_byte(0x012c) << 8) | (read_cmos_sensor_byte(0x012d) & 0xFFFF));
