...
static imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0xa340b362,//0xb1893b4f,
...
static kal_uint32 return_sensor_id(void)
{
    ...
    //kal_uint32 tmp = 0;
    // return (tmp = read_cmos_sensor(0x0F17)) ? ((tmp << 8) | read_cmos_sensor(0x0F16)) : 0;
}
...
static int HI843B_otp_read(void)
{
	...
	LOG_INF("[OTP_Module_Info] addr = 0x%x \n", addr);
	
	if (addr == 0) {
		HI843B_otp.module_integrator_id = 0;
		HI843B_otp.AF_Flag = 0;
		HI843B_otp.production_year = 0;
		...
		checksum = HI843B_otp.module_integrator_id = OTP_read_cmos_sensor(addr);
		HI843B_otp.AF_Flag = OTP_read_cmos_sensor(addr + 1);
		checksum = checksum + HI843B_otp.AF_Flag;
		
		HI843B_otp.production_year = OTP_read_cmos_sensor(addr + 2);
		checksum = checksum + HI843B_otp.production_year;
		
		HI843B_otp.production_month = OTP_read_cmos_sensor(addr + 3);
		checksum = checksum + HI843B_otp.production_month;
		
		HI843B_otp.production_day = OTP_read_cmos_sensor(addr + 4);
		checksum = checksum + HI843B_otp.production_day;
		
		HI843B_otp.sensor_id = OTP_read_cmos_sensor(addr + 5);
		checksum = checksum + HI843B_otp.sensor_id;
		
		HI843B_otp.lens_id = OTP_read_cmos_sensor(addr + 6);
		checksum = checksum + HI843B_otp.lens_id;
		
		HI843B_otp.vcm_id = OTP_read_cmos_sensor(addr + 7);
		checksum = checksum + HI843B_otp.vcm_id;
		
		HI843B_otp.Driver_ic_id = OTP_read_cmos_sensor(addr + 8);
		checksum = checksum + HI843B_otp.Driver_ic_id;
		
		HI843B_otp.F_num_id = OTP_read_cmos_sensor(addr + 9);
		checksum = checksum + HI843B_otp.F_num_id;
		
		HI843B_otp.infocheck = OTP_read_cmos_sensor(addr + 16);

		hw_info_main_otp.otp_valid = 1;
		hw_info_main_otp.vendor_id = HI843B_otp.module_integrator_id;
		hw_info_main_otp.year = HI843B_otp.production_year;
		hw_info_main_otp.month = HI843B_otp.production_month;
		hw_info_main_otp.day = HI843B_otp.production_day;
		hw_info_main_otp.lens_id = HI843B_otp.lens_id;
		hw_info_main_otp.vcm_id = HI843B_otp.vcm_id;
	}
	...
	{
		LOG_INF("[OTP_Module_Info]: Module information checksum PASS\n ");
	}
	else
	{
		LOG_INF("[OTP_Module_Info]: Module information checksum Fail\n ");
	}


	LOG_INF("[OTP_Module_Info] module_integrator_id = 0x%x\n", HI843B_otp.module_integrator_id);
	LOG_INF("[OTP_Module_Info] AF_Flag = 0x%x\n", HI843B_otp.AF_Flag);
	LOG_INF("[OTP_Module_Info] production_year = 0x%x\n", HI843B_otp.production_year);
	LOG_INF("[OTP_Module_Info] production_month = 0x%x\n", HI843B_otp.production_month);
	LOG_INF("[OTP_Module_Info] production_day = 0x%x\n", HI843B_otp.production_day);
	LOG_INF("[OTP_Module_Info] sensor_id = 0x%x\n", HI843B_otp.sensor_id);
	LOG_INF("[OTP_Module_Info] lens_id = 0x%x\n", HI843B_otp.lens_id);
	LOG_INF("[OTP_Module_Info] vcm_id = 0x%x\n", HI843B_otp.vcm_id);
	LOG_INF("[OTP_Module_Info] Driver_ic_id = 0x%x\n", HI843B_otp.Driver_ic_id);
	LOG_INF("[OTP_Module_Info] F_num_id = 0x%x\n", HI843B_otp.F_num_id);
	LOG_INF("[OTP_Module_Info] infocheck = 0x%x, checksum = 0x%x\n", HI843B_otp.infocheck, checksum);
	...
	if (HI843B_otp.WB_FLAG == 0x01)
		...
		LOG_INF("[OTP_Module_Info]WB data invalid \n");


	LOG_INF("[OTP_Module_Info] WB_FLAG = 0x%x; wb_start_addr= 0x%x\n", 
		HI843B_otp.WB_FLAG, wb_start_addr);
	
	//LOG_INF("[OTP_Module_Info] wb_start_addr = 0x%x \n", wb_start_addr);
	...
	if (wb_start_addr != 0) {
		...
		write_HynixOtp_sensor_16(0x70a, wb_start_addr);
		HI843B_OTP_write_cmos_sensor(0x702, 0x01);	//single mode
		for (i = 0; i < 30; i++) {
			HI843B_otp.wb_data[i] = read_cmos_sensor(0x708);	//otp data read
			HI843B_otp.wbcheck = HI843B_otp.wb_data[i] + HI843B_otp.wbcheck;
			
			LOG_INF("[OTP_Module_Info] HI843 wb_data[%d] = 0x%x\n", i, HI843B_otp.wb_data[i]);
		}
		
		HI843B_otp.wbcheck = HI843B_otp.wbcheck - HI843B_otp.wb_data[29];
		
		if((HI843B_otp.wbcheck % 0x100) == HI843B_otp.wb_data[29])
			LOG_INF("[OTP_Module_Info] wbcheck = 0x%x, checksum = 0x%x, WB checksum OK!! ", HI843B_otp.wbcheck, HI843B_otp.wb_data[29]);
		else
			LOG_INF("[OTP_Module_Info] wbcheck = 0x%x, checksum = 0x%x, WB checksum ERROR!! ", HI843B_otp.wbcheck, HI843B_otp.wb_data[29]);
	}
...
static int HI843B_otp_apply(void)
{
	...
	int RG_ratio_unit = 0;
	int BG_ratio_unit = 0;
	int RG_ratio_golden = 0x13E;//0x13F
	int BG_ratio_golden = 0x122;//0x124
	LOG_INF("[OTP_Module_Info] HI843B_otp_apply.\n");
	...
	LOG_INF("[OTP_Module_Info] RG_ratio_golden = 0x%x, BG_ratio_golden = 0x%x", RG_ratio_golden, BG_ratio_golden);
	LOG_INF("[OTP_Module_Info] RG_ratio_unit = 0x%x, BG_ratio_golden = 0x%x", RG_ratio_unit, BG_ratio_unit);
	...
	LOG_INF("[OTP_Module_Info] Before apply otp G_gain = 0x%x\n", (read_cmos_sensor(0x0508) << 8) | (read_cmos_sensor(0x0509) & 0xFFFF));
	LOG_INF("[OTP_Module_Info] Before apply otp R_gain = 0x%x\n", (read_cmos_sensor(0x050c) << 8) | (read_cmos_sensor(0x050d) & 0xFFFF));
	LOG_INF("[OTP_Module_Info] Before apply otp B_gain = 0x%x\n", (read_cmos_sensor(0x050e) << 8) | (read_cmos_sensor(0x050f) & 0xFFFF));
...
{
	LOG_INF("[OTP_Module_Info] otp_cali hi843b_otp_read_flag = 0x%d\n",hi843b_otp_read_flag);
...
write_cmos_sensor(0x0002, 0x0000); 
write_cmos_sensor(0x0a02, 0x0100); 
write_cmos_sensor(0x0a04, 0x015a);  //  turn off:0x014a  turn on:0x015a
write_cmos_sensor(0x0036, 0x0070); 
write_cmos_sensor(0x0038, 0x7000); 
write_cmos_sensor(0x004e, 0x7070); 
write_cmos_sensor(0x075a, 0x0000); 
...
write_cmos_sensor(0x0024, 0x0036); 
write_cmos_sensor(0x002a, 0x0045); 
write_cmos_sensor(0x0026, 0x017a);  //  72 + 306 = 378
write_cmos_sensor(0x002c, 0x08a5); //   2519 - 306 = 2213
write_cmos_sensor(0x005c, 0x0202); 
write_cmos_sensor(0x002e, 0x1111); 
write_cmos_sensor(0x0030, 0x1111); 
write_cmos_sensor(0x0032, 0x1111); 
...
write_cmos_sensor(0x0a22, 0x0000); 
write_cmos_sensor(0x0a12, 0x0cc0); 
write_cmos_sensor(0x0a14, 0x072c); //1826
write_cmos_sensor(0x003c, 0x0006); 
write_cmos_sensor(0x003e, 0x0000); 
write_cmos_sensor(0x0004, 0x09dc); 
write_cmos_sensor(0x0057, 0x0000); 
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	enable_TP = ((read_cmos_sensor(0x0A04) << 8) | read_cmos_sensor(0x0A05));  //0x015a
	enable_TP &= 0xFFE7;  
	if (enable) { 
	...
	LOG_INF("[Hi843b_test_pattern_mode] enable_TP = 0x%x\n", enable_TP);
