...
static imgsensor_info_struct imgsensor_info = { 
	...
	.i2c_addr_table = {0x6C,0xff},// 0x42 record sensor support all write id addr, only supprt 4must end with 0xff
};
...
/*No Need to implement this function*/ 
 ...
 static int ov8856_otp_read_flag = 0;
...
static int RG_Ratio_Typical= 0x12C ; //0x14C
static int BG_Ratio_Typical= 0x154 ; //0x130
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			*sensor_id =  (read_cmos_sensor(0x300B) << 8 )| read_cmos_sensor(0x300C);
			...
			LOG_INF("Read sensor id fail, i2c:0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	if(ov8856_otp_read_flag == 0)
	{
		read_otp(&otp_ptr);
		ov8856_otp_read_flag = 1;
	}
