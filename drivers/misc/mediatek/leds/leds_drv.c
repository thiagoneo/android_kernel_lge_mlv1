...
//+fanmingxing rgbr2015/11
extern int mtkrgbR_gpio_init(struct platform_device *pdev);
//+fanmingxing rgbr2015/11
...
#endif
//+fanmingxing rgbr2015/11
static int red_led_inited = 0;
//-fanmingxing rgbr2015/11

/****************************************************************************
 * driver functions
 ***************************************************************************/
extern struct pinctrl *rgbRctrl; //fanmingxing 2015/12
extern struct pinctrl_state *main_rgbR_h ; //fanmingxing 2015/12
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		//+fanmingxing rgbr2015/11
		if((!strncmp(cust_led_list[i].name, "red", 3)) && (0 == red_led_inited) ) {
			printk("[WENDELL]-%s: is called, red\n", __func__);

			#if !defined(CONFIG_MTK_LEGACY)
			   mtkrgbR_gpio_init(pdev);
    		#endif
	 ...
	 //	pinctrl_select_state(rgbRctrl, main_rgbR_h);//fanmingxing 2015/12
		/*
	mt_set_gpio_mode(GPIO10|0X80000000,GPIO_MODE_00);
   	mt_set_gpio_dir(GPIO10|0X80000000,GPIO_DIR_OUT);
     	mt_set_gpio_out(GPIO10|0X80000000,GPIO_OUT_ONE);*/
...
//+fanmingxing rgbr2015/11
#ifdef CONFIG_OF
static const struct of_device_id Kd_leds_of_ids[] = {
...
static struct platform_driver mt65xx_leds_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
		   #ifdef CONFIG_OF
		   .of_match_table = Kd_leds_of_ids,  //+fanmingxing rgbr2015/11
   		   #endif
		   },
