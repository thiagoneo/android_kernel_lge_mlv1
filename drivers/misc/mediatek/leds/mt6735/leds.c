...
char *leds_name[MT65XX_LED_TYPE_TOTAL] = {
	...
	"button-backlight",
	"lcd-backlight",
	"torch",                        //fanmingxing 2015/11 torch
};
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	switch (pmic_type) {
	case MT65XX_LED_PMIC_NLED_ISINK0:
			#if defined(CONFIG_HQ_BREATHE_LIGHT_EFFECT)
...
// +fanmingxing 2015/11/23 led_gpio
struct pinctrl *rgbRctrl = NULL;
struct pinctrl_state *main_rgbR_l = NULL;
struct pinctrl_state *main_rgbR_h = NULL;
int mtkrgbR_gpio_init(struct platform_device *pdev)
{
       int ret = 0;
       rgbRctrl = devm_pinctrl_get(&pdev->dev);
       if (IS_ERR(rgbRctrl)) {
               dev_err(&pdev->dev, "Cannot find rgbr pinctrl!");
               ret = PTR_ERR(rgbRctrl);
			   printk("[WENDELL]-%s: rgbRctrl init failed\n", __func__);
       }

       main_rgbR_l = pinctrl_lookup_state(rgbRctrl, "main_rgbR_mode0");
       if (IS_ERR(main_rgbR_l)) {
               ret = PTR_ERR(main_rgbR_l);
               pr_debug("%s : pinctrl err, main_flashlight_mode0\n", __func__);
			   printk("[WENDELL]-%s: main_rgbR_l init failed\n",__func__);
       }
       
       main_rgbR_h = pinctrl_lookup_state(rgbRctrl, "main_rgbR_mode1");
       if (IS_ERR(main_rgbR_h)) {
               ret = PTR_ERR(main_rgbR_h);
               pr_debug("%s : pinctrl err, main_rgbR_mode1\n", __func__);
			   printk("[WENDELL]-%s: main_rgbR_h init failed\n",__func__);
       } 
       return ret;
}
EXPORT_SYMBOL(mtkrgbR_gpio_init);
static int chr_det_led_control(int level)
{
   if(level)
   	{
	 pinctrl_select_state(rgbRctrl, main_rgbR_l);
   	}
   else
   	{
   	pinctrl_select_state(rgbRctrl, main_rgbR_h);
   	}
     
   return 0;
}
// -fanmingxing 2015/11/23 led_gpio
extern struct pinctrl *flashlightctrl;
extern struct pinctrl_state *main_torch_pwm;
extern struct pinctrl_state *main_torch_gpio;
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_PWM:
		if (strcmp(cust->name, "lcd-backlight") == 0|| (strcmp(cust->name,"torch") == 0)) {
			if(strcmp(cust->name, "torch") == 0)
			pinctrl_select_state(flashlightctrl, main_torch_pwm);//torch_pwm fanmingxing2015/11
			//mt_set_gpio_mode(GPIO_CAMERA_FLASH_MODE_PIN, GPIO_CAMERA_FLASH_MODE_PIN_M_PWM);
			bl_brightness_hal = level;
			if (level == 0) {
				mt_pwm_disable(cust->data,cust->config_data.pmic_pad);
				if(strcmp(cust->name,"torch") == 0)
					{
					pinctrl_select_state(flashlightctrl, main_torch_pwm);//torch_pwm fanmingxing2016/1/25
		...
		pinctrl_select_state(flashlightctrl, main_torch_gpio);//torch_gpio fanmingxing2015/12
			if (level == 0) {
	...
	case MT65XX_LED_MODE_GPIO:
		...
		// +fanmingxing 2015/11/23 led_gpio
		return chr_det_led_control(level);
		// -fanmingxing 2015/11/23 led_gpio
