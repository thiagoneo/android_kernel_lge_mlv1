...
static ssize_t ltr559_show_ps(struct device_driver *ddri, char *buf)
{
	...
	if((res = ltr559_ps_read(ltr559_obj->client, &ltr559_obj->ps)))
	{
		return sprintf(buf,"ERROR: %d\n", res);
	}
	else
	{
		if (ltr559_obj->ps > MAX_PS_NOISE)
			return sprintf(buf,"device_name:%s @FAIL@ psensor_noise = %d\n", LTR559_DEV_NAME, ltr559_obj->ps);
		else
...
//static DRIVER_ATTR(als_lowpower,   0664,);
/*----------------------------------------------------------------------------*/
...
static int ltr559_dynamic_calibrate(void)
{
	...
	ltr559_i2c_write_reg(LTR559_PS_MEAS_RATE, 0x02); 
...
static int ltr559_psensor_calibrate(void)
{
	...
	if(sum < 11){
		atomic_set(&obj->ps_thd_val_high,obj->hw->ps_threshold_high-0+obj->current_xtalk);
		atomic_set(&obj->ps_thd_val_low,obj->hw->ps_threshold_low-0+obj->current_xtalk);		
	}else if(sum < 20){
		atomic_set(&obj->ps_thd_val_high,obj->hw->ps_threshold_high+8+obj->current_xtalk);
		atomic_set(&obj->ps_thd_val_low,obj->hw->ps_threshold_low+3+obj->current_xtalk);
	}else{
		atomic_set(&obj->ps_thd_val_high,obj->hw->ps_threshold_high+10+obj->current_xtalk);
		atomic_set(&obj->ps_thd_val_low,obj->hw->ps_threshold_low+5+obj->current_xtalk);
	}
...
static int ltr559_als_enable(struct i2c_client *client, int enable)
{
	...
	regdata = init_als_gain;//ltr559_i2c_read_reg(LTR559_ALS_CONTR);

	if (enable == 1) {
		APS_LOG("ALS(1): enable als only %x \n",regdata);
		regdata |= 0x01;
	} else {
		APS_LOG("ALS(0): disable als only %x \n",regdata);
		regdata &= 0xfe;
	}
...
static int ltr559_devinit(void)
{
	...
	res = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 4); // 3--> 4
	if(res<0)
	{
...
static int ltr559_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if((err = ltr559_check_and_clear_intr(client)))
	{
