...
#define AP3426
#ifdef AP3426
#define HQ_PS_CALIBRATION
#ifdef HQ_PS_CALIBRATION
	#define MAX_PS_NOISE 650
	static int current_crosstalk;
	static int initial_noise;
	static int current_noise ;
#endif
...
int Huaqin_alreadey_enable = 1;//dade
...
static int ap3xx6_set_pcrosstalk(struct i2c_client *client, int val)
{
	...
	atomic_set(&obj->ps_thd_val_h, obj->hw->ps_threshold_high + val);
	atomic_set(&obj->ps_thd_val_l, obj->hw->ps_threshold_low + val);

	err = ap3xx6_set_plthres(client, atomic_read(&obj->ps_thd_val_l));
	err = ap3xx6_set_phthres(client, atomic_read(&obj->ps_thd_val_h));
...
#if 1
/*******************************************/
	ap3xx6_set_PSTtime(client, 0x08);//  0x25  bit5~0
	ap3xx6_set_PSgain(client, 0x01);

	ap3xx6_set_PSpulse(client, 0x03);

	ap3xx6_set_meantime(client, 0x02);//  0x23 bit1~0
...
#ifdef HQ_PS_CALIBRATION
int ap3xx6_init_calibration(struct i2c_client *client)
{
	int err;
	int i = 0;
	int count = 0;
	...
	if(count != 0){
		ps_data = ps_data/count;
	}

	if (ps_data > MAX_PS_NOISE)
		ps_data = 0;
...
err_out:
	APS_ERR("ap3xx6_init_calibration fail\n");
	return 0;
}
...
static int ap3xx6_Calibration_every_time(struct i2c_client *client)
{
	...
	if(count != 0){
		ui_ps_data = ui_ps_data/count;
	}
	
	if (ui_ps_data > MAX_PS_NOISE) {
		current_crosstalk = initial_noise;	
		current_noise = initial_noise;
	} else {

		current_crosstalk = ui_ps_data;
		
		if(initial_noise == 0)
			initial_noise = current_crosstalk;
		
		APS_LOG("%s OK ,current_crosstalk is %d \n ",__func__,current_crosstalk);
	}
	
	current_noise = ui_ps_data;
	
	ap3xx6_set_pcrosstalk(obj->client, current_crosstalk);
...
err_out:
	APS_ERR("ap3xx6_Calibration_every_time fail\n");
	return -1;
}
...
static void ap3xx6_eint_work(struct work_struct *work)
{
	...
	//	sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
		...
		printk("PHB: Report PS State: %d\n", ps_value);
		if(ap3xx6_init_flag == 0){
...
/*
static ssize_t ap3xx6_Huaqin_show_val(struct device_driver *ddri, char* buf)//dade
{
	unsigned char int_status;
	unsigned char data[2]={0};
	int pstatus = 0;
	int ret = 0;
	//ap3426_pls_t *pls = container_of(to_delayed_work(work),ap3426_pls_t, ps_work);
	if(Huaqin_alreadey_enable)
{
	int_status = ap3xx6_read_reg(ap3xx6_obj->client, AP3xx6_INT_STATUS, 0xFF, 0x00);
	data[0] = ap3xx6_read_reg(ap3xx6_obj->client, AP3xx6_PDATA_L, 0xFF, 0x00);
	data[1] = ap3xx6_read_reg(ap3xx6_obj->client, AP3xx6_PDATA_H, 0xFF, 0x00);
	pstatus = ((data[1]&0x03)<<8) + (data[0]) ;
	//pstatus = ((data[1]&0x03)<<8) + (data[0]) + prev_cal_data;     
	//pobj = (pstatus > pls->ps_lth) ? 1 : 0;  
	ret = sprintf(buf, "AP3426 PS val=%d\n",pstatus);
}
else
{
 	ret = sprintf(buf, " %s please enable Psensor\n","error");

} 	

	return ret;
}*/
static ssize_t ap3426_Huaqin_show_val(struct device_driver *ddri, char *buf)
{
...
#if 1 //phb add 0330
	count = sprintf(buf,"AP3426 PS val=%d\n", current_noise);
#else
	int reg_value[2] = {0, 0};
	if (!ap3xx6_obj) {
		APS_ERR("ap3xx6_obj is null!!\n");
	...
	reg_value[0] = ap3xx6_read_reg(ap3xx6_obj->client, 0x28, 0xFF, 0x00);
	reg_value[1] = ap3xx6_read_reg(ap3xx6_obj->client, 0x29, 0xFF, 0x00);
	if ((reg_value[0] < 0) || (reg_value[1] < 0)) {
		APS_ERR("i2c read_reg err\n");
		return -1;
	}

	count = sprintf(buf,"AP3426 PS val=%d\n", reg_value[0] + (reg_value[1]<<8));
#endif		
	return count;
}
//static DRIVER_ATTR(em,	S_IWUGO | S_IRUGO, ap3xx6_em_read, ap3xx6_em_write);
static DRIVER_ATTR(Huaqin_show_val, S_IRUGO, ap3426_Huaqin_show_val, NULL);
...
//static DRIVER_ATTR(Huaqin_show_val, S_IRUGO, ap3xx6_Huaqin_show_val, NULL);//dade
...
static struct driver_attribute *ap3xx6_attr_list[] = {
	...
	&driver_attr_status,
	&driver_attr_Huaqin_show_val,//LIXUNWEI
	//&driver_attr_,
	//&driver_attr_trace,
};
...
static int ap3xx6_ps_enable_nodata(int en)
{
	...
	ap3xx6_Calibration_every_time(obj->client);
#endif
		ap3xx6_write_reg(obj->client, 0x02, 0xFF, 0x00, 0x80);	  //enable PS interrupt
		msleep(50);
		...
		clear_bit(CMC_BIT_PS, &obj->enable);
    ap3xx6_write_reg(obj->client, 0x02, 0xFF, 0x00, 0x00);        //disable PS interruptap3xx6_set_pcrosstalk(obj->client, 511);
...
APS_LOG("als_register_data_path OK.%s:\n", __func__);
	...
	msleep(100);
	initial_noise = ap3xx6_init_calibration(client);
