...
static int KXTJ2_1009_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	{
		GSE_LOG("Sensor power status is newest!\n");
#ifdef CONFIG_HQ_PROJECT_ZAW670
	//return KXTJ2_1009_SUCCESS;
#else
	return KXTJ2_1009_SUCCESS;
#endif
...
//char *accdev_name = KXTJ2_1009_DEV_NAME;
...
/*Kionix Auto-Cali End*/
	...
	{
		GSE_ERR("ReadSensorData in suspend value=%d", atomic_read(&obj->suspend));
		return 0;
	}
...
static ssize_t show_direction_value(struct device_driver *ddri, char *buf)
{
	...
	if(obj->hw)
	{
		len += sprintf(buf, "direction %d\n",obj->hw->direction);   
	}
	else
	{
		len += sprintf(buf, "CUST: NULL\n");
	}
	return len;    
}
...
static ssize_t store_direction_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if(1 == sscanf(buf, "0x%x", &direction))
	{
...
static DRIVER_ATTR(direction, 0664, show_direction_value, store_direction_value);
...
static int kxtj2_1009_resume(struct i2c_client *client)
{
...
#endif
    //mutex_lock(&kxtj2_1009_mutex);    //phb modified 2016.4.7
#ifdef CUSTOM_KERNEL_SENSORHUB
...
#endif
	{
		...
		//mutex_unlock(&kxtj2_1009_mutex);    //phb modified 2016.4.7
		return err;        
	}
...
static int kxtj2_1009_get_data(int* x ,int* y,int* z, int* status)
{
		...
		int err = 0;
#else
	char buff[KXTJ2_1009_BUFSIZE] = {0};
#endif
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_gsensor_name = KXTJ2_1009_DEV_NAME;	
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
