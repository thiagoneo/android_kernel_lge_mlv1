...
//#include "HQ_ZAL1066_LG_CTPFW_MSG5846_TXD_EVT_0E.02.h"
//#include "HQ_ZAL1066_LG_CTPFW_MSG5846_DJ_EVT_08.03_160825.h"
#include "HQ_1066_MSG5846_DJ_PVT_0308.0014_V1.0_170109.h"
        ...
        if (bIsCorrectFormat)
        {
            ...
            switch (nWakeupMode)
            {
                case 0x58:
                    ...
                    input_report_key(g_InputDevice, KEY_GESTURE_U, 1);
                    input_sync(g_InputDevice);
                    input_report_key(g_InputDevice, KEY_GESTURE_U, 0);
                    input_sync(g_InputDevice);
                    break;	
				#if 0
                case 0x60:
...
//                    input_report_key(g_InputDevice, RESERVER51, 0);
        ...
        {
            DBG(&g_I2cClient->dev, "gesture wakeup packet format is incorrect.\n");
            printk("gesture wakeup packet format is incorrect.\n");
...
#ifdef SUPPORT_Z_SIZE_REPORT
                nSize = ( (pPacket[(4*i)+4]));
                pInfo->tPoint[pInfo->nCount].nP = nSize;
#else         
...
void DrvFwCtrlOpenGestureWakeup(u32 *pMode)
{
    ...
    while (i < 5)
    {
        ...
        if (rc > 0)
        {
			...
			printk("Enable gesture wakeup success\n");
            break;
        }
    ...
    if (i == 5)
    {
    	printk("Enable gesture wakeup failed\n");
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    if (nRetVal == 0)
    {
		...
		sprintf(g_tpd_module_name,"DJ_%02x.%02x_A",0x08,nMinor&0xff);
		sprintf(ctp_fw_info, "CTPFW_msg2xxx_DJ_08.14_A");
		#endif
