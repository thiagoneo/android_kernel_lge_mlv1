...
extern u8 g_hq_ctp_module_fw_version;
   ...
   {
	      //#include "HQ_HX8527E44_zal1066_firmware.i"
	        #include "HQ_AL1066_HX8527E44_Booyi_A.0x10.0x12_20161227.i"
	
   };
   static unsigned char i_CTPM_FW_B[]=
   {
	      #include "HQ_HX8527E44_zal1066_firmware_0b.i"
   };
...
static int i_update_FW(void)
{
	...
	int machine_ver = get_machine_version();

	if (machine_ver <= 1){
		if ( private_ts->vendor_config_ver != 0x0b) { // 1 means DVT1
			force_upgrade = 1;
		}
		ImageBuffer = i_CTPM_FW_B;
		fullFileLength = sizeof(i_CTPM_FW_B);
	} else {
		ImageBuffer = i_CTPM_FW;
		fullFileLength = sizeof(i_CTPM_FW);
	}
	I("private_ts->vendor_config_ver = %x, force_upgrade = %d,  line = %d\n", private_ts->vendor_config_ver, force_upgrade, __LINE__);
	...
	if (( private_ts->vendor_fw_ver_H < ImageBuffer[FW_VER_MAJ_FLASH_ADDR] )
		...
		|| ( himax_calculateChecksum(false) == 0 ) || force_upgrade)
		{
update_retry:
...
static void himax_cable_detect_func(void)
{
	...
	ts = private_ts;
	//I("Touch: cable status%d, ts->usb_connected = %d\n", connect_status, ts->usb_connected);
	if (ts->cable_config) {
...
#endif
#endif
	sprintf(g_tpd_module_name,"BY_%02x.%02x_A", 0x10, private_ts->vendor_config_ver);//0x10: boyi
	sprintf(ctp_fw_info, "CTPFW_Himax852xes_BY_10.12_A");
	/*
