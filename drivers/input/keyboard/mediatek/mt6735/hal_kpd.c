...
#define TARGET_BUILD_PWRLOS   1

#ifdef TARGET_BUILD_PWRLOS
//#ifdef CONFIG_TARGET_BUILD_PWRLOSS
...
#ifdef CONFIG_KPD_PMIC_LPRST_TD	
#ifdef CONFIG_META_PWRKEY_LONG_PRESS_POWEROFF
	else if(get_boot_mode() == META_BOOT) {			//WENDELL-20140804 added
		
		kpd_print("[WENDELL]-Meta Mode long press reboot selection\n");
		pmic_set_register_value(PMIC_RG_PWRKEY_RST_EN, 0x01);
		pmic_set_register_value(PMIC_RG_HOMEKEY_RST_EN, 0x00);
		pmic_set_register_value(PMIC_RG_PWRKEY_RST_TD, CONFIG_KPD_PMIC_LPRST_TD);
	}
