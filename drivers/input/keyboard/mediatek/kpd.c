...
#endif
/***********************************/
#ifdef CONFIG_HUAQIN_LONGPRESS_EXT_KEY			//WENDELL-20140804 added
#include <mach/mt_rtc_hw.h>
#include <mach/mtk_rtc_hal.h>
#define HUAQIN_LONGPRESS_EXT_KEY_TIME 1 // ray modify the time according to D3
static struct hrtimer hq_extkey_timer;
static bool hq_extkey_timer_started = false;
static u32 ext_keycode = 0;
#endif
...
#ifdef CONFIG_HUAQIN_LONGPRESS_EXT_KEY
void huaqin_kpd_longpress_ext_key_handler(u32 keycode, u16 pressed)
{
	kpd_print("%s key:%d,pressed:%d \n",__func__,keycode,pressed);
	ext_keycode = keycode;
	if(keycode == KEY_VOLUMEDOWN){
		if(pressed == 1){
			wake_lock_timeout(&kpd_suspend_lock,msecs_to_jiffies(5*1000));
			hq_extkey_timer_started = true;
			hrtimer_start(&hq_extkey_timer, 
						ktime_set(HUAQIN_LONGPRESS_EXT_KEY_TIME, 0),
						HRTIMER_MODE_REL);
			kpd_print("[zqq]-hq_extkey_timer start \n");
		}else{
			if(hq_extkey_timer_started){
				hrtimer_cancel(&hq_extkey_timer);
				hq_extkey_timer_started = false;
				kpd_print("[zqq]-hq_extkey_timer stop \n");
			}	
		}
	}
...
static enum hrtimer_restart hq_ext_key_timer_handler(struct hrtimer *timer)
{
	kpd_print("[zqq]-hq_ext_key_timer_handler \n");
	if(mt_get_bl_brightness() == 0){
		if(ext_keycode == KEY_VOLUMEDOWN){
			input_report_key(kpd_input_dev,KEY_HQ_EXT_KEY1,1);// press
			input_sync(kpd_input_dev);
			mdelay(100);
			input_report_key(kpd_input_dev,KEY_HQ_EXT_KEY1,0);//release
			input_sync(kpd_input_dev);
		}
		else if(ext_keycode == KEY_VOLUMEUP){
			input_report_key(kpd_input_dev,KEY_HQ_EXT_KEY2,1);// press
			input_sync(kpd_input_dev);
			mdelay(100);
			input_report_key(kpd_input_dev,KEY_HQ_EXT_KEY2,0);//release
			input_sync(kpd_input_dev);
		}
	}
	return HRTIMER_NORESTART;
...
static void kpd_keymap_handler(unsigned long data)
{
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			input_sync(kpd_input_dev);
			#ifdef CONFIG_HUAQIN_LONGPRESS_EXT_KEY
			huaqin_kpd_longpress_ext_key_handler(linux_keycode,pressed);
			#endif
...
#ifdef CONFIG_HUAQIN_LONGPRESS_EXT_KEY
	__set_bit(KEY_HQ_EXT_KEY1, kpd_input_dev->keybit);
	__set_bit(KEY_HQ_EXT_KEY2, kpd_input_dev->keybit);
#endif
#if KPD_AUTOTEST
...
#ifdef CONFIG_HUAQIN_LONGPRESS_EXT_KEY
	hrtimer_init(&hq_extkey_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	hq_extkey_timer.function = hq_ext_key_timer_handler;
...
static int kpd_pdrv_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	#ifndef CONFIG_HUAQIN_LONGPRESS_EXT_KEY
